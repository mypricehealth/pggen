...
type Querier interface {
	...
	// CountAuthors returns the number of authors (zero params).
	QueueCountAuthors(batch *pgx.Batch) *QueuedCountAuthors

	// FindAuthorById finds one (or zero) authors by ID (one param).
	QueueFindAuthorByID(batch *pgx.Batch, params FindAuthorByIDParams) *QueuedFindAuthorByID

	// InsertAuthor inserts an author by name and returns the ID (two params).
	QueueInsertAuthor(batch *pgx.Batch, params InsertAuthorParams) *QueuedInsertAuthor
...
func (q *DBQuerier) QueueCountAuthors(batch *pgx.Batch) *QueuedCountAuthors {
...
func (q *DBQuerier) QueueFindAuthorByID(batch *pgx.Batch, params FindAuthorByIDParams) *QueuedFindAuthorByID {
	err := registerTypes(context.Background(), q.conn)
	if err != nil {
		panic(q.errWrap(fmt.Errorf("could not register types: %w", err)))
	}
...
func (q *DBQuerier) QueueInsertAuthor(batch *pgx.Batch, params InsertAuthorParams) *QueuedInsertAuthor {
	err := registerTypes(context.Background(), q.conn)
	if err != nil {
		panic(q.errWrap(fmt.Errorf("could not register types: %w", err)))
	}
